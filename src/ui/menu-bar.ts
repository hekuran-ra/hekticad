...
const MENUS: Record<MenuId, { label: string; entries: MenuEntry[] }> = {
  ...
  einfuegen: {
    ...
    entries: [
      ...
      { id: 'insert:dim',      label: 'Bemaßung Punkt-zu-Punkt', shortcut: () => toolShortcutLabel('dim'),   action: () => activateTool('dim') },
      { id: 'insert:dim_h',    label: 'Bemaßung horizontal',     shortcut: () => toolShortcutLabel('dim_h'), action: () => activateTool('dim_h') },
      { id: 'insert:dim_v',    label: 'Bemaßung vertikal',       shortcut: () => toolShortcutLabel('dim_v'), action: () => activateTool('dim_v') },
